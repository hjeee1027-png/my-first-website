...
export default function SizeGuideModal({ onClose, user }) {
  ...
  const aiDesc = result
    ? (result.fit === 'exact'
      ? `${result.size} 사이즈가 체형 데이터에 딱 맞습니다. 슬림핏 제품도 부담 없이 선택하실 수 있어요.`
      : `체형 분석 결과 ${result.size} 사이즈를 권장드립니다. 여유로운 핏을 원하신다면 한 치수 업도 고려해보세요.`)
    : ''
            ...
            {result && (
              <div className={styles.resultWrap}>
                ...
                <p className={styles.resultDesc}>
                  비슷한 체형의 고객 구매 데이터 및 피드백을 AI가 분석한 결과입니다.
                </p>
                <div className={styles.feedbackBtn}>{feedbackText}</div>
