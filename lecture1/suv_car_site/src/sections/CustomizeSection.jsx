...
export default function CustomizeSection() {
  ...
  return (
    <Box sx={{ bgcolor: '#0c121c', py: { xs: 8, md: 12 } }}>
        ...
        <Grid container spacing={4}>
          ...
          <Grid item xs={12} md={3.6}>
              ...
              {MODELS.map((m, i) => (
                <Box
                  ...
                  sx={{
                    p: 2,
                    mb: 1,
                    border: `1px solid ${selectedModel === i ? '#A68966' : 'rgba(74,74,74,0.3)'}`,
                    cursor: 'pointer',
                    display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                    bgcolor: selectedModel === i ? 'rgba(166,137,102,0.05)' : 'transparent',
                    transition: 'all 0.2s',
                    '&:hover': { borderColor: 'rgba(166,137,102,0.5)' },
                  }}
                >
                  <Box>
                    <Typography sx={{ color: selectedModel === i ? '#fff' : '#4A4A4A', fontSize: '0.875rem', fontWeight: selectedModel === i ? 600 : 400 }}>
                      {m.shortName}
                    </Typography>
                  </Box>
            ...
            <Box sx={{ mb: 4 }}>
                  ...
                  <Tooltip key={c.id} title={c.nameKo} placement="top">
                    <Box
                      ...
                      sx={{
                        width: 32, height: 32,
                        bgcolor: c.hex,
                        border: selectedColor === c.id ? '2px solid #A68966' : '2px solid rgba(255,255,255,0.1)',
                        cursor: 'pointer',
                        position: 'relative',
                        transition: 'all 0.2s',
                        '&:hover': { transform: 'scale(1.15)', border: '2px solid rgba(166,137,102,0.7)' },
                      }}
                    >
                      {selectedColor === c.id && (
              ...
              <Typography sx={{ color: '#4A4A4A', fontSize: '0.8rem', mt: 1 }}>{color?.nameKo}</Typography>
            </Box>
              ...
              {WHEEL_OPTIONS.map((w) => (
                <Box
                  ...
                  sx={{
                    p: 1.5,
                    mb: 1,
                    border: `1px solid ${selectedWheel === w.id ? '#A68966' : 'rgba(74,74,74,0.3)'}`,
                    cursor: 'pointer',
                    display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                    bgcolor: selectedWheel === w.id ? 'rgba(166,137,102,0.05)' : 'transparent',
                    '&:hover': { borderColor: 'rgba(166,137,102,0.5)' },
                    transition: 'all 0.2s',
                  }}
                >
                  <Box>
                    <Typography sx={{ color: selectedWheel === w.id ? '#fff' : '#4A4A4A', fontSize: '0.85rem', fontWeight: selectedWheel === w.id ? 600 : 400 }}>
                      {w.name}
                    </Typography>
            ...
            <Box sx={{ mb: 4 }}>
              ...
              <Button
                ...
                sx={{ borderColor: '#4A4A4A', color: '#4A4A4A', '&:hover': { borderColor: '#A68966', color: '#A68966' }, py: 1 }}
              >
                미리보기 적용
              </Button>
            </Box>
          ...
          <Grid item xs={12} md={8.4}>
            ...
            >
              ...
              <Box sx={{ position: 'relative', mb: 3, textAlign: 'center' }}>
                ...
                >
                  {/* 차량 SVG with dynamic color */}
                  <svg viewBox="0 0 700 300" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', maxHeight: 280, filter: 'drop-shadow(0 20px 60px rgba(0,0,0,0.9))' }}>
                    {/* 그림자 */}
                    <ellipse cx="350" cy="265" rx="260" ry="18" fill="rgba(0,0,0,0.5)" />
                    {/* 차체 */}
                    <path d="M80 200 L80 135 Q85 112 125 95 L210 72 Q245 62 300 58 L400 58 Q455 62 490 72 L580 95 Q615 112 620 135 L620 200 Z" fill={color?.hex || '#0c121c'} />
                    {/* 차체 하이라이트 */}
                    <path d="M95 155 Q200 145 350 142 Q500 145 605 155" stroke="rgba(255,255,255,0.08)" strokeWidth="1.5" fill="none" />
                    {/* 루프 */}
                    <path d="M195 95 Q240 52 320 44 L380 44 Q460 52 505 95" fill={color?.hex || '#0c121c'} stroke="rgba(255,255,255,0.05)" strokeWidth="1" />
                    {/* 윈드실드 */}
                    <path d="M192 97 Q238 58 320 48 L320 97 Z" fill="rgba(100,160,220,0.15)" stroke="rgba(166,137,102,0.25)" strokeWidth="0.8" />
                    <path d="M508 97 Q462 58 380 48 L380 97 Z" fill="rgba(100,160,220,0.15)" stroke="rgba(166,137,102,0.25)" strokeWidth="0.8" />
                    {/* 사이드 윈도우 */}
                    <rect x="330" y="62" width="90" height="35" rx="2" fill="rgba(100,160,220,0.12)" stroke="rgba(166,137,102,0.2)" strokeWidth="0.8" />
                    {/* 도어 라인 */}
                    <line x1="322" y1="97" x2="322" y2="196" stroke="rgba(255,255,255,0.06)" strokeWidth="0.8" />
                    <line x1="432" y1="97" x2="432" y2="196" stroke="rgba(255,255,255,0.06)" strokeWidth="0.8" />
                    {/* 그릴 */}
                    <rect x="83" y="148" width="28" height="38" rx="1" fill="rgba(10,14,22,0.8)" stroke="#A68966" strokeWidth="0.8" />
                    ...
                    <path d="M86 128 Q93 120 112 118 L126 121 Q116 126 86 133 Z" fill="rgba(255,240,190,0.3)" />
                    {/* 테일라이트 */}
                    <path d="M618 128 Q610 118 585 115 L568 118 Q580 125 618 135 Z" fill="rgba(200,40,40,0.08)" stroke="rgba(200,40,40,0.4)" strokeWidth="0.8" />
                    <path d="M614 128 Q607 120 588 118 L574 121 Q584 126 614 133 Z" fill="rgba(200,40,40,0.25)" />
                    {/* 로고 */}
                    <text x="350" y="140" textAnchor="middle" fill="rgba(166,137,102,0.6)" fontSize="11" fontWeight="bold" letterSpacing="4" fontFamily="Roboto, sans-serif">VANTAGE</text>
                    {/* 앞바퀴 */}
                    <circle cx="192" cy="203" r="52" fill="#0a0a0a" stroke="#333" strokeWidth="2" />
                    <circle cx="192" cy="203" r="40" fill={color?.hex === '#F5F5F5' ? '#ddd' : '#111'} stroke="#A68966" strokeWidth="1.5" />
                    <circle cx="192" cy="203" r="20" fill="#0a0a0a" />
                    ...
                    {/* 뒷바퀴 */}
                    <circle cx="508" cy="203" r="52" fill="#0a0a0a" stroke="#333" strokeWidth="2" />
                    <circle cx="508" cy="203" r="40" fill={color?.hex === '#F5F5F5' ? '#ddd' : '#111'} stroke="#A68966" strokeWidth="1.5" />
                    <circle cx="508" cy="203" r="20" fill="#0a0a0a" />
                    ...
                    {showMonogram && monogram && (
                      <text x="350" y="190" textAnchor="middle" fill="rgba(166,137,102,0.8)" fontSize="22" fontWeight="bold" letterSpacing="6" fontFamily="Roboto, sans-serif" style={{ fontStyle: 'italic' }}>
                        {monogram}
                      </text>
                    )}
                    {showMonogram && monogram && (
                      <text x="350" y="215" textAnchor="middle" fill="rgba(166,137,102,0.3)" fontSize="9" letterSpacing="2" fontFamily="Roboto, sans-serif">
                        PERSONAL EDITION
                      </text>
                    )}
                  </svg>
                ...
                {showMonogram && monogram && (
                  <Chip
                    ...
                    sx={{ bgcolor: 'rgba(166,137,102,0.1)', color: '#A68966', border: '1px solid rgba(166,137,102,0.3)', borderRadius: 0, mt: 1, fontSize: '0.75rem' }}
                  />
                )}
              </Box>

              {/* 선택 요약 + 가격 */}
              <Box sx={{ borderTop: '1px solid rgba(74,74,74,0.3)', pt: 3 }}>
                <Grid container spacing={2} sx={{ mb: 3 }}>
                  <Grid item xs={12} sm={4}>
                    <Typography sx={{ color: '#4A4A4A', fontSize: '0.75rem', letterSpacing: '0.1em', mb: 0.5 }}>모델</Typography>
                    <Typography sx={{ color: '#fff', fontSize: '0.9rem', fontWeight: 500 }}>{model.shortName}</Typography>
                  </Grid>
                  <Grid item xs={12} sm={4}>
                    <Typography sx={{ color: '#4A4A4A', fontSize: '0.75rem', letterSpacing: '0.1em', mb: 0.5 }}>색상</Typography>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      <Box sx={{ width: 14, height: 14, bgcolor: color?.hex, border: '1px solid rgba(255,255,255,0.2)' }} />
                      <Typography sx={{ color: '#fff', fontSize: '0.9rem', fontWeight: 500 }}>{color?.nameKo}</Typography>
                    </Box>
                  </Grid>
                  <Grid item xs={12} sm={4}>
                    <Typography sx={{ color: '#4A4A4A', fontSize: '0.75rem', letterSpacing: '0.1em', mb: 0.5 }}>휠</Typography>
                    <Typography sx={{ color: '#fff', fontSize: '0.9rem', fontWeight: 500 }}>{wheel?.name}</Typography>
                  </Grid>
                </Grid>

                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', mb: 3 }}>
                  <Box>
                    <Typography sx={{ color: '#4A4A4A', fontSize: '0.75rem', letterSpacing: '0.1em', mb: 0.5 }}>예상 금액</Typography>
                    <Typography sx={{ color: '#A68966', fontWeight: 700, fontSize: { xs: '1.5rem', md: '1.8rem' } }}>
                      {formatPrice(finalPrice)}
                    </Typography>
                    <Typography sx={{ color: '#4A4A4A', fontSize: '0.75rem' }}>
                      부가세 포함 · 옵션 추가 금액 별도
                    </Typography>
                  </Box>
                  <Box sx={{ display: 'flex', gap: 1 }}>
                    <IconButton onClick={handleShare} sx={{ color: '#4A4A4A', '&:hover': { color: '#A68966' }, border: '1px solid rgba(74,74,74,0.3)' }}>
                      <ShareIcon fontSize="small" />
                    </IconButton>
                    <IconButton sx={{ color: '#4A4A4A', '&:hover': { color: '#A68966' }, border: '1px solid rgba(74,74,74,0.3)' }}>
                ...
                <Box sx={{ display: 'flex', gap: 2 }}>
                  <Button
                    ...
                    sx={{ bgcolor: '#A68966', color: '#0B0B0B', border: 'none', '&:hover': { bgcolor: '#c4a882' }, py: 1.8, fontWeight: 600 }}
                  >
                    이 구성으로 시승 예약
                  </Button>
                  <Button
                    ...
                    sx={{ borderColor: '#A68966', color: '#A68966', py: 1.8 }}
                  >
                    견적 문의
                  </Button>
                </Box>
